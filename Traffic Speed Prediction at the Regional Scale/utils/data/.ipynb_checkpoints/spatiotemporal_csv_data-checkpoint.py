...
class SpatioTemporalCSVDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        if self.model_name == "SceneGCN":
            (
                self.train_dataset,
                self.val_dataset,
            ) = utils.data.functions.generate_SceneGCN_torch_datasets(
                self._feat,
                self.seq_len,
                self.pre_len,
    ...
    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=len(self.val_dataset), num_workers=4)
