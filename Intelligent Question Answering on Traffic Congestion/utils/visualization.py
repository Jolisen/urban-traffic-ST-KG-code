...
def save_map_as_image(html_path, img_path):
    ...
    driver_path = PROJECT_ROOT / "tools" / "chromedriver-win64" / "chromedriver.exe"
...
def plot_traffic_congestion(honeycomb_ids_ls, road_ids_ls, poi_ids_ls, honeycomb_list, roadcrs, g, plot_type): 
    ...
    if plot_type == 'grid congestion':
        ...
        for batch_name, group in grouped:
            ...
            # **保存 HTML & 转换为 png**
            output_folder = PROJECT_ROOT / "maps" / "grid_congestion"
            output_folder.mkdir(parents=True, exist_ok=True)

            html_path = output_folder / f"{batch_name.replace(':', '_')}.html"
            img_path  = output_folder / f"{batch_name.replace(':', '_')}.png"
            m.save(str(html_path))
            save_map_as_image(str(html_path), str(img_path))
            
            cropped_img_with_title_path = output_folder / f"{batch_name.replace(':', '_')}_cropped_with_title.png"
            crop_and_add_title(str(img_path), str(cropped_img_with_title_path), batch_name)
    ...
    if plot_type == 'road congestion':
        ...
        # 绘图
            ...
            # **保存 HTML & 转换为 png**
            output_folder = PROJECT_ROOT / "maps" / "road_congestion"
            output_folder.mkdir(parents=True, exist_ok=True)
